...
public interface AthleteAvailabilityRepository extends ReactiveCrudRepository<AthleteAvailability, Long> {
    @Query("SELECT * FROM athlete_availability WHERE athlete_id = :athleteId AND date BETWEEN :start AND :end ORDER BY date, start_time")
    Flux<AthleteAvailability> findByAthleteIdAndDateBetween(Long athleteId, LocalDate start, LocalDate end);

    @Query("SELECT * FROM athlete_availability WHERE athlete_id = :athleteId AND date = :date ORDER BY start_time")
